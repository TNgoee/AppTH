        ...
        child: Row(
          children: [
            Container(
              ...
              child: Text(breed.id.toString(), style: const TextStyle(fontSize: 16.0, fontWeight: FontWeight.bold)),
            ),
            const SizedBox(width: 20.0),
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [Text(breed.name, style: const TextStyle(fontSize: 18.0, fontWeight: FontWeight.w500)), const SizedBox(height: 4.0), Text(breed.desc)],
              ),
            ),
            IconButton(
              onPressed: () {
                setState(() {
                  DatabaseService.instance.deleteCategory(breed.id!);
                });
              },
            ...
            IconButton(
              onPressed: () {
                setState(() {
                  Navigator.of(
                    context,
                  ).push(MaterialPageRoute(builder: (_) => CategoryAdd(isUpdate: true, categoryModel: breed), fullscreenDialog: true)).then((_) => setState(() {}));
                });
              },
